...
export class GitHubIssuesTrigger implements TriggerPlugin {
  ...
  async validateWebhook(req: WebhookRequest): Promise<boolean> {
    ...
    if (!signature) {
      logger.warn('[GitHubIssuesTrigger] No signature provided - rejecting webhook');
      return false;
    }
    ...
    if (repoFullName) {
      ...
      const project = await projects.findByRepo(repoFullName);
    ...
    const secret = await githubAuth.getWebhookSecret(projectId);
    if (!secret) {
      logger.error('[GitHubIssuesTrigger] GITHUB_WEBHOOK_SECRET not configured' +
  ...
  async parseWebhook(payload: unknown): Promise<TriggerEvent | null> {
    ...
    if (await this.isBotSender(sender?.login)) {
      logger.info(`[GitHubIssuesTrigger] Ignoring event from bot: ${sender?.login}`);
      return null;
    }
    ...
    logger.info(`[GitHubIssuesTrigger] Ignoring action: ${action}`);
    return null;
  }
  ...
  private async parseIssueEvent(payload: GitHubIssueEventPayload): Promise<TriggerEvent | null> {
    ...
    if (!issue || !repository) {
      logger.error('[GitHubIssuesTrigger] Missing issue or repository data');
      return null;
    }
    ...
    if (!project) {
      logger.info(`[GitHubIssuesTrigger] No project configured for repo: ${repoFullName}`);
      return null;
    }

    const githubConfig = project.triggers.github;
    if (!githubConfig?.issues) {
      logger.info(`[GitHubIssuesTrigger] GitHub issues trigger not enabled for: ${repoFullName}`);
      return null;
    }
  ...
  private async parseCommentEvent(payload: GitHubIssueCommentEventPayload): Promise<TriggerEvent | null> {
    ...
    if (!comment || !issue || !repository) {
      logger.error('[GitHubIssuesTrigger] Missing comment, issue, or repository data');
      return null;
    }
    ...
    if (!isPRComment) {
      ...
      if (!githubConfig?.commentTrigger) {
        logger.info(`[GitHubIssuesTrigger] Comment trigger not configured for: ${repoFullName}`);
        return null;
      }
  ...
  private async parsePRReviewEvent(payload: GitHubPullRequestReviewEventPayload): Promise<TriggerEvent | null> {
    ...
    if (!pull_request || !repository) {
      logger.error('[GitHubIssuesTrigger] Missing pull_request or repository data');
      return null;
    }
  ...
  private async parsePRLabeledEvent(payload: GitHubPullRequestEventPayload): Promise<TriggerEvent | null> {
    ...
    if (!isPlanApproved && !isEscalation) {
      logger.info(`[GitHubIssuesTrigger] Ignoring label: ${labelName}`);
      return null;
    }

    if (isPlanApproved) {
      logger.info(`[GitHubIssuesTrigger] Plan approved via label on PR #${pull_request.number}`);
    } else {
      logger.info(`[GitHubIssuesTrigger] Escalation label ${labelName} added to PR #${pull_request.number}`);
    }
  ...
  async shouldProcess(event: TriggerEvent): Promise<boolean> {
    ...
    if (state !== 'open') {
      logger.info(`[GitHubIssuesTrigger] Skipping ${state} issue: ${event.triggerId}`);
      return false;
    }
  ...
  async updateStatus(event: TriggerEvent, status: FixStatus): Promise<void> {
    ...
    if (!token) {
      logger.error('[GitHubIssuesTrigger] No GitHub App installation for repo:', { repoFullName });
      return;
    }
    ...
    try {
      ...
      logger.info(`[GitHubIssuesTrigger] Posted ${status.fixed ? 'success' : 'failure'} comment to issue #${issueNumber}`);
    } catch (error: unknown) {
      logger.error(`[GitHubIssuesTrigger] Failed to post comment:`, { error: getErrorMessage(error) });
    }
  }
  ...
  async addComment(event: TriggerEvent, comment: string): Promise<void> {
    ...
    if (!owner || !repo || !targetNumber) {
      logger.error('[GitHubIssuesTrigger] Missing required fields for comment:', { owner, repo, issueNumber, prNumber });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(`[GitHubIssuesTrigger] GitHub API error ${response.status}:`, { errorBody });
        return;
      }

      logger.info(`[GitHubIssuesTrigger] Posted comment to ${owner}/${repo}#${targetNumber}`);
    } catch (error: unknown) {
      logger.error(`[GitHubIssuesTrigger] Failed to post comment:`, { error: getErrorMessage(error) });
    }
  }
