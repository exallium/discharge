...
async function prepareClaudeConfig(workspacePath: string, projectId?: string): Promise<ClaudeConfigPaths> {
  ...
  logger.debug(`[ClaudeCode] Claude config written to ${claudeJsonFile}`);
...
async function getSecret(
  ...
  return secrets.getSecret(plugin, key, projectId, envFallback);
}
...
export class ClaudeCodeRunner implements RunnerPlugin {
  ...
  async run(options: RunOptions): Promise<RunResult> {
    ...
    try {
      if (useWorktrees) {
        ...
        logger.debug(`[ClaudeCode:${jobId}] Worktree created at ${workspacePath}`);
      } else {
        // Traditional clone approach
        const worktreeDir = process.env.WORKTREE_DIR || '/workspaces';
        workspacePath = `${worktreeDir}/${jobId}`;

        logger.debug(`[ClaudeCode:${jobId}] Cloning repository...`);
        ...
        // Create fix branch
        fixBranch = `fix/auto-${jobId.slice(0, 8)}`;
        logger.debug(`[ClaudeCode:${jobId}] Creating branch ${fixBranch}`);
      ...
      if (oauthToken) {
        logger.debug(`[ClaudeCode:${jobId}] Using CLAUDE_CODE_OAUTH_TOKEN`);
      } else {
        logger.debug(`[ClaudeCode:${jobId}] Using ANTHROPIC_API_KEY`);
      }
      ...
      logger.info(`[ClaudeCode:${jobId}] Running Claude Code CLI...`);
      ...
      logger.info(`[ClaudeCode:${jobId}] Execution completed`);
      ...
      if (hasCommit) {
        logger.info(`[ClaudeCode:${jobId}] Found commit: ${gitLog.trim()}`);
      }
      ...
      try {
        ...
        if (analysis) {
          logger.info(`[ClaudeCode:${jobId}] Analysis:`, {
            canAutoFix: analysis.canAutoFix,
            confidence: analysis.confidence,
          });
        }
      } catch {
        logger.debug(`[ClaudeCode:${jobId}] No analysis.json found`);
      }
      ...
      try {
        ...
        if (triageResult) {
          logger.info(`[ClaudeCode:${jobId}] Triage:`, {
      ...
      try {
        ...
        if (investigationResult) {
          logger.info(`[ClaudeCode:${jobId}] Investigation:`, {
      ...
      logger.error(`[ClaudeCode:${jobId}] Execution failed:`, { error: errorMessage });
  ...
  private async writeToolsToWorkspace(
    ...
    logger.debug(`[ClaudeCode] Generated ${tools.length} tools in ${toolsDir}`);
  }
  ...
  async runConversation(
    ...
    logger.info(`[ClaudeCode:${jobId}] Starting conversation mode execution`, {
      repo: options.repoUrl,
      branch: options.branch,
    ...
    try {
      if (isPreConfiguredWorkspace) {
        ...
        logger.debug(`[ClaudeCode:${jobId}] Cloning repository (branch: ${cloneBranch})...`);
        const authUrl = await getAuthenticatedRepoUrl(options.repoUrl);
        ...
        try {
          ...
          if (errorMsg.includes('Remote branch') && errorMsg.includes('not found')) {
            ...
            logger.warn(`[ClaudeCode:${jobId}] Branch ${cloneBranch} not found, falling back to ${fallbackBranch}`);
          } else {
        ...
        if (branchNotFound) {
          const fallbackBranch = options.defaultBranch || 'main';
          logger.info(`[ClaudeCode:${jobId}] Cloning default branch ${fallbackBranch} instead...`);
          await execAsync(
          ...
          fixBranch = options.existingPrBranch || `fix/conversation-${jobId.slice(0, 8)}`;
          logger.info(`[ClaudeCode:${jobId}] Creating new branch ${fixBranch} (original branch was deleted)`);
          await execAsync(`git checkout -b ${fixBranch}`, { cwd: workspacePath });

          // Push the recreated branch to remote so it exists for GitHub API operations
          logger.info(`[ClaudeCode:${jobId}] Pushing recreated branch to remote...`);
          ...
          logger.debug(`[ClaudeCode:${jobId}] Using existing PR branch: ${fixBranch}`);
        } else {
          ...
          await execAsync(`git checkout -b ${fixBranch}`, { cwd: workspacePath }).catch(() => {
            ...
            logger.debug(`[ClaudeCode:${jobId}] Branch already exists, continuing...`);
          });
        }
      }
      ...
      // Run Claude Code (read prompt from file inside container via cat pipe)
      logger.info(`[ClaudeCode:${jobId}] Running Claude Code CLI in conversation mode...`);
      const { stdout } = await execAsync(
        `
        docker run --rm \
      ...
      logger.info(`[ClaudeCode:${jobId}] Conversation execution completed`);
      ...
      logger.error(`[ClaudeCode:${jobId}] Conversation execution failed:`, { error: fullError });
      ...
      logger.debug(`[ClaudeCode:${jobId}] Error classified as: ${errorType}, requiresAdmin: ${requiresAdmin}`);
  ...
  private async parseConversationResult(
    ...
    try {
      ...
      logger.debug('[ClaudeCode] Found conversation-result.json, parsing...');
      ...
      logger.debug('[ClaudeCode] No conversation-result.json found, parsing from output');
    }

    // Log stdout length for debugging
    logger.debug(`[ClaudeCode] Parsing from stdout (${stdout.length} chars)`);
  ...
  private validateAndFixPlan(
    ...
    if (hasEmptySections && stdout && stdout.trim()) {
      ...
      const logger = getLogger();
      logger.debug('[ClaudeCode] Plan has empty sections, using stdout as fallback');
    ...
    if (fallbackContext || fallbackApproach) {
      const logger = getLogger();
      logger.debug('[ClaudeCode] Filled plan with fallback content from stdout', {
